...
$(function() {
	$('#accordion').on('click', function(e) {
		if (e.target.tagName == 'A') {
			...
			$.post('/pxzb/System/system_getOneOfUser', {
				"user.userId" : id
			}, function(xhr) {
				$.post('/pxzb/System/system_getAllDepartment', '', function(xhr2) {
		...
		var that = $.confirm({
			...
			buttons : {
				updateUser : {
					...
					text : '修改',
					action : function() {
						$.post('/pxzb/System/system_modifyUser', $('#user_update').serialize(), function(xhr_data) {
				...
				resetUser : {
					...
					text : '重置密码',
					action : function() {
						$.post('/pxzb/System/system_resetPassword', $('#user_update').serialize(), function(xhr_data) {
							if (xhr_data.length > 0) {
						...
						$.alert({
							...
							buttons : {
								Delete : {
									...
									text : '确认',
									action : function() {
										$.post('/pxzb/System/system_deleteUser', $('#user_update').serialize(), function(xhr_data) {
											if (xhr_data == "success") {
												toastr.success("删除成功");
												that.close();
												//重新刷新该页面
			...
			$.confirm({
				...
				buttons : {
					deleteUser : {
						...
						text : '添加',
						action : function() {
							$.post('/pxzb/System/system_setAdminUser', this.$content.find('#user_add').serialize(), function(xhr_data) {
								if (xhr_data.length > 0) {
									toastr.success("添加成功");
									//重新刷新该页面
									$('.right-side').load('page/administrator/adminAccountManagement.jsp #content', function() {
