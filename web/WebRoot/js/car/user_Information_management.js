$(function() {
	...
	var getinfoByCardIdb = function() {
		...
		if (reg.test(card_num) === false) {
			...
			if (parseInt(card_num.substr(16, 1)) % 2 == 1) {
				$('select[name="zbInfo.sex"]').find("option[value='男']").attr("selected", "selected");
			//是男则执行代码 ...
			} else {
				$('select[name="zbInfo.sex"]').find("option[value='女']").attr("selected", "selected");
			//是女则执行代码 ...
			}
			$('input[name="zbInfo.birthday"]').val(card_num.substring(6, 10) + "-" + card_num.substring(10, 12) + "-" + card_num.substring(12, 14));
		}
		...
		$.confirm({
			...
			buttons : {
				deleteUser : {
					...
					text : '确认',
					action : function() {
						$.post('/pxzb/zb/zb_userPuchInfoToadmin', {
							tableId : id,
	...
	var sure_export = function() {
		 ...
		 */
		if (a_href == "info") {
			data.export_id = $('input[name="zbInfo.zbInfoId"]').val() + ",";
		} else {
		...
		if (data.export_id != "" && data.export_name != "") {
			...
			location.href = "/pxzb/zb/zb_userExportExcelCollection?tableName=" + data.tableName + "&export_id=" + (data.export_id).substring(0, data.export_id.length - 1) + "&export_name=" + (data.export_name).substring(0, data.export_name.length - 1);
		} else {
			toastr.error("未选择数据");
		}
		data.export_id = "";
	...
	var modiInfo = function() {
		...
		var id = $(this).siblings('input').val();
		//查询单条信息
		$.post("/pxzb/zb/zb_userGetTableInfoByTableId",
			{
				tableId : id,
				tableName : data.tableName
			}, function(xhr) {
				...
				$.each(xhr.attachmentName, function(i, v) {
					...
					$("#" + modal_id + " .addInfo").before('<div class="img-default">' + '<div class="img">'
						+ '<img src="/pxzb/System/system_Attachment?attachmentName=' + v + '!' + data.tableName + '" alt="" class="img-show">'
						+ '</div>'
	...
	$('.nav-tabs li a').click(function() {
		...
		//infoType，除去zb前部分
		infoType = data.tableName.replace("zb", "");
		...
		$.post("/pxzb/zb/zb_userSetTableInfo", review_data, function(sxh_data) {
			if (sxh_data.result == "success") {
				...
				if ($("#" + modal_id + " .img-default").length > 0) {
					...
					//附件上传
					if (AttachmentUpload("/pxzb/zb/zb_userAttachmentUpload", formData)) {
						toastr.success("上传成功!");
						$("#" + modal_id).modal('hide');
						doQuery();
					}
	...
	$('input[name$="UserNames"]').keyup(function() {
		...
		$.post('/pxzb/zb/zb_getUserIdOrderingByUserName', {
	...
	$('.search_info').click(function() {
		...
		if (this_object.text().trim() == "确认搜索") {
			...
			this_object.siblings('#search_input').find('div').each(function() {
				name = data.tableName.replace("zb", "zb") + '.' + $(this).attr('id').replace("Inputu", "");
				var val_arr = [];
	...
	function doQuery() {
		$.ajax({
			url : "/pxzb/zb/zb_userGetTableInfoInPaging",
			type : "post",
			async : false,
			timeout : 3000,
			data : data,
	...
	function getStr(xhr) {
		...
		case 'info':
			...
			//设置用户名
			$('input[name="zbInfo.userName"]').val($('.userName_info').text());
