$(function() {
	$('.table-responsive').on('click', function(e) {
		if (e.target.tagName == 'A') {
			...
			$.post('/pxzb/Admin/admin_getOneOfIntroduction', {
				tableId : id
			}, function(xhr) {
		...
		$.confirm({
			...
			buttons : {
				modifiyUser : {
					...
					text : '修改',
					action : function() {
						$.post('/pxzb/Admin/admin_modifyIntroduction', $('#Introduction_update').serialize(), function(xhr_data) {
				...
				deleteinfo : {
					...
					text : '删除',
					action : function() {
						$.post('/pxzb/Admin/admin_deleteIntroduction', $('#Introduction_update').serialize(), function(xhr_data) {
							if (xhr_data.result == "success") {
								toastr.success("删除成功");
								toIntroductionModify();
							} else {
		...
		$.confirm({
			...
			buttons : {
				deleteUser : {
					...
					text : '添加',
					action : function() {
						$.post('/pxzb/Admin/admin_setIntroduction', $('#Introduction_add').serialize(), function(xhr_data) {
