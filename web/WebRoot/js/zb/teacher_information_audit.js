...
$(function() {
		...
		$.post("/pxzb/Admin/admin_getzbTableInfoById",
			{
				...
				$("#" + modal_id).find('.sure_mod').unbind().click(function() {
					...
					$.post("/pxzb/Admin/admin_modifiedInfomation", review_data, function(sxh_data) {
	...
	var solidInfo = function() {
		...
		$(this).parent().empty().append('<img title="已出库"  src="img/ok1.png" />');
		...
		case 'info':
			for (i = 0; i < xhr.length; i++) {
				...
				str += '<td><input type="hidden" value="' + xhr[i][0].zbInfoId + '" >'
					...
					+ '<button class="btn btn-default btn-xs solidButton" title="出库"><i class="fa fa-chain fa-lg" ></i></button>'
					+ '</td></tr>';
				str += "</tr>";
			}
			break;
