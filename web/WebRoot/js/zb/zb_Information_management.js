...
$(function() {
	...
	var modiUserInfo = function() {
		...
		//查询单条用户的信息
		$.post("/pxzb/Admin/admin_getzbTableInfoById",
			{
				tableId : id,
				...
				$('#info_modal').find('.sure_mod').unbind().click(function() {
					...
					$.post("/pxzb/zb/zb_userSetTableInfo", review_data, function(sxh_data) {
		...
		$.post("/pxzb/Admin/admin_getzbTableInfoById",
			{
				...
				//modal_id_1，除去zb前部分，方便后部分操作
				var modal_id_1 = data.tableName.replace("zb", "");
	...
	var relieveInfo = function() {
		...
		$.post('/pxzb/Admin/admin_LiftCuring', {
			tableId : infoid,
			tableName : data.tableName,
			dataState : "10"
		}, function(xhr) {
	...
	var sure_export = function() {
		...
		if (data.export_id != "" && data.export_name != "") {
			window.open("/pxzb/Admin/admin_ExportExcelCollection?tableName=" + data.tableName + "&export_id=" + (data.export_id).substring(0, data.export_id.length - 1) + "&export_name=" + (data.export_name).substring(0, data.export_name.length - 1));
		//location.href = "/pxzb/Admin/admin_ExportExcelCollection?tableName=" + data.tableName + "&export_id=" + (data.export_id).substring(0, data.export_id.length - 1) + "&export_name=" + (data.export_name).substring(0, data.export_name.length - 1);
		} else {
			toastr.error("未选择数据");
			return;
		}
	...
	var all_sure_export = function() {
		...
		if (data.export_name != "") {
			window.open("/pxzb/Admin/admin_ExportExcelCollection?tableName=" + data.tableName + "&export_name=" + (data.export_name).substring(0, data.export_name.length - 1));
		//location.href = "/pxzb/Admin/admin_ExportExcelCollection?tableName=" + data.tableName + "&export_id=" + (data.export_id).substring(0, data.export_id.length - 1) + "&export_name=" + (data.export_name).substring(0, data.export_name.length - 1);
		} else {
			toastr.error("未选择数据");
			return;
	...
	$('.nav-tabs li a').click(function() {
		...
		data.tableName = "zb" + a_href.substring(0, 1).toUpperCase() + a_href.substring(1);
		//执行查询操作
		doQuery();
		//导出模态框初始化
		m_check.init({
	...
	$('.search_info').click(function() {
		...
		if (this_object.text().trim() == "确认搜索") {
			...
			this_object.siblings('#search_input').find('div').each(function() {
				name = data.tableName.replace("zb", "zb") + '.' + $(this).attr('id');
				if (name == "zbInfo.userName") {
					name = "user.userName";
				}
				var val_arr = [];
	...
	$('.sure_add_info').click(function() {
		...
		$.post("/pxzb/Admin/admin_addzbInfo", review_data, function(sxh_data) {
			if (sxh_data.result == "success") {
				toastr.success('添加成功!')
			} else {
				toastr.error('添加失败!');
	...
	function doQuery() {
		$.ajax({
			url : "/pxzb/Admin/admin_getSpecifiedInformationByPaging",
			type : "post",
			async : false,
			timeout : 3000,
	...
	function getStr(xhr) {
		...
		case 'info':
			for (i = 0; i < xhr.length; i++) {
				...
				str += '<td><input type="hidden" value="' + xhr[i][0].zbInfoId + '" >'
