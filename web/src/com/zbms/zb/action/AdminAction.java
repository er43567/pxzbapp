...
public class AdminAction extends ActionSupport {
	...
	// 通过ID获取单条信息
	public void getzbTableInfoById() {
		try {
			HttpServletResponse response = ServletActionContext.getResponse();
			TableInfoAndUserVo list = adminService.getzbTableInfoById(tableName, tableId);
			response.setContentType("text/html;charset=utf-8");
			response.getWriter().write(new Gson().toJson(list));
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void addzbInfo() {
		try {
			HttpServletResponse response = ServletActionContext.getResponse();
			String result = adminService.addzbInfo(zbInfo, getSecondaryCollegeInfo("id"));
			response.setContentType("text/html;charset=utf-8");
	...
	public void importDatabase() {
		try {
			...
			if (fileFileName.contains("奖励")) {
				propertiesName = "z_zb_award";
				cla = zbAward.class;
			} else if (fileFileName.contains("信息")) {
				propertiesName = "z_zb_info";
				cla = zbInfo.class;
			} else if (fileFileName.contains("论文")) {
				propertiesName = "z_zb_paper";
				cla = zbPaper.class;
			} else if (fileFileName.contains("专利")) {
				propertiesName = "z_zb_patent";
				cla = zbPatent.class;
			} else if (fileFileName.contains("项目")) {
				propertiesName = "z_zb_project";
				cla = zbProject.class;
			} else if (fileFileName.contains("著作")) {
				propertiesName = "z_zb_works";
				cla = zbWorks.class;
			} else {
			}
			XSSFWorkbook workbook = new XSSFWorkbook(is);
			List<Object> list = ExcelToBean2.toObjectPerproList(
					ExcelToBean2.parseUpdateExcel(workbook, "com.zbms.all.domain." + propertiesName), cla);
