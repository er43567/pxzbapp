...
public class zbDaoImpl implements zbDao {
	...
	public TableInfoAndUserVo getTableInfoByTableId(String tableName, String tableInfoIdName, String tableId) {
		String hql = "select new com.zbms.zb.vo.TableInfoAndUserVo(t,u) from " + tableName
	...
	@Override
	public TableInfoAndUserVo getzbInfoByUserId(String userId) {
		String hql = "select new com.zbms.zb.vo.TableInfoAndUserVo(t,u) from zbInfo t,User u where t.userId=u.userId and t.userId = '"
				+ userId + "'";
		System.out.println(hql);
		return (TableInfoAndUserVo) getSession().createQuery(hql).uniqueResult();
	}
	 ...
	 */
	@Override
	public String updateInfo(zbInfo zbInfo) {
		try {
			getSession().merge(zbInfo);
			getSession().update(zbInfo);
		} catch (Exception e) {
		}
		return "success";
	}
